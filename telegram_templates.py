"""Telegram message templates for Cable AI Scalp v1.2
...
def _split_banner(banner: str) -> tuple[str, str]:
    ...
    Handles both:
      ...
      'Cable AI Scalp v1.2 | GBP/USD' → ('Cable AI Scalp v1.2', 'GBP/USD')
    """
...
def msg_signal_update(
    ...
    h4_adx=None, h4_trending=True, max_score=6,
) -> str:
    bot, pair = _split_banner(banner)
    s_str = f"{score}/{max_score}"
    ...
    def _adx_line():
        if h4_adx is None: return ""
        icon = "✅" if h4_trending else "🚫"
        status = "trending" if h4_trending else "ranging ⚠️"
        return f"H4 ADX: {icon} {h4_adx:.1f}  ({status})\n"
...
def msg_trade_opened(
    ...
    h1_trend="UNKNOWN", h1_aligned=True, max_score=6,
) -> str:
    bot, pair = _split_banner(banner)
...
def msg_ai_guard_result(action, risk_level, reason, score=None, direction="", pair="GBP/USD", mode="TRADE", max_score=6) -> str:
    """Telegram card for optional AI News Guard.

    AI is a risk guard only. It does not create signals, change score,
    change position size, or override the fixed calendar hard-lock.
    """
    action = str(action or "ALLOW").upper()
    risk = str(risk_level or "LOW").upper()
    reason = str(reason or "No elevated news risk detected.").strip()[:240]
    direction = str(direction or "").upper()
    score_line = f"Signal: Score {score}/{max_score} {direction}\n" if score is not None else ""

    if action == "BLOCK":
        title = "🚫 AI News Guard BLOCKED trade"
        final = "Action: No trade placed"
    elif action == "CAUTION":
        title = "🧠 AI News Guard: CAUTION"
        final = "Action: Trade allowed with caution" if str(mode).upper() != "ALERT_ONLY" else "Action: Watch only"
    else:
        title = "🧠 AI News Guard: ALLOW"
        final = "Action: Trade allowed" if str(mode).upper() != "ALERT_ONLY" else "Action: Watch only"
...
def msg_news_penalty(event_names, penalty, score_after, score_before,
                     position_after, position_before, max_score=6) -> str:
    ...
    return (
        f"📰 News Penalty\n{_DIV}\n"
        f"Event:    {names}\n"
        f"Score:    {score_before}/{max_score} → {score_after}/{max_score}  (penalty {penalty:+d})\n"
        f"Position: {pos}\n"
        f"{status}"
    )
...
def msg_startup(
    ...
    tokyo_fresh_cross_min_score=None, max_score=6,
    h4_adx_enabled=True, h4_adx_period=14, h4_adx_threshold=20.0,
) -> str:
    ...
    ai_line = (
        f"AI guard: 🧠 ON | Extra GBP/USD news-risk filter | {ai_news_guard_model}\n"
        if ai_news_guard_enabled else
        "AI guard: OFF\n"
    )
    adx_line = (
        f"H4 ADX:   {'✅' if h4_adx_enabled else '⬜'} "
        f"{'ON' if h4_adx_enabled else 'OFF'}  (H4, period {h4_adx_period}, threshold {h4_adx_threshold:.0f})\n"
    )
    return (
        f"🚀 {version} started\n{_DIV}\n"
        f"Mode:      {mode}  |  Balance: ${balance:,.2f}\n"
        f"Pair:      GBP/USD (Cable)\n"
        f"Strategy:  M5 EMA + ORB + CPR + AI News Guard  |  Cycle: {cycle_minutes} min\n"
        f"Min score: {min_score}/{max_score}  |  Alerts: score ≥{tg_min_score} only\n"
