...
Active sessions: London 16–20 SGT, US Cont. 00–03 SGT, Tokyo 08–15 SGT
(entry requires score ≥ signal_threshold, default 4; max score derived from score_weights)
...
from signals import SignalEngine, score_to_position_usd, compute_max_score
...
from telegram_templates import (
    ...
    msg_margin_adjustment, msg_session_open, msg_ai_guard_result,
    _clean_session,
)
...
def _build_sessions(settings: dict) -> list:
    ...
    sessions = [
        ("Tokyo Window",  "Tokyo",  tok_s, tok_e, 4),
...
def _build_signal_checks(score, direction, rr_ratio=None, tp_pct=None,
                         ...
                         tp_min_pct=0.35, max_score=6):
    mandatory_checks = [
        (f"Score >= {signal_threshold}",
         score >= signal_threshold and direction != "NONE", f"{score}/{max_score}"),
        (f"RR >= {min_rr_ratio:.1f}",
         None if rr_ratio is None else rr_ratio >= min_rr_ratio,
         "n/a" if rr_ratio is None else f"{rr_ratio:.2f}"),
    ]
    quality_checks = [
        (f"TP >= {tp_min_pct:.2f}%", None if tp_pct is None else tp_pct >= tp_min_pct,
...
def validate_settings(settings: dict) -> dict:
    """Validate the already-merged settings dict.

    Defaults are applied upstream in config_loader.load_settings() from the
    single canonical source (bundled settings.json) — this function only checks
    that required keys exist and values are sane. It no longer defines defaults,
    so there is exactly one place any default value lives.
    """
    required = ["pairs"]  # pair_sl_tp fixed pips used exclusively
    ...
    if int(settings.get("signal_threshold", 4)) > compute_max_score(settings):
        raise ValueError(
            "signal_threshold ({}) exceeds max achievable score ({}); "
            "no trade could ever fire.".format(
                settings.get("signal_threshold"), compute_max_score(settings))
        )
...
def compute_tp_price_dist(levels: dict, sl_price_dist: float, settings: dict) -> float:
    ...
    if rec is not None:
        try:
            ...
            pass
    return round(sl_price_dist * float(settings.get("rr_ratio", 1.67)), 8)  # emergency RR fallback
...
def derive_rr_ratio(levels: dict, sl_price_dist: float, tp_price_dist: float, settings: dict) -> float:
    ...
    return float(settings.get("rr_ratio", 1.67))
...
def _guard_phase(db, run_id, settings, alert, history, now_sgt, today, demo,
    ...
    log.info("=== %s | %s | %s SGT ===",
             settings.get("bot_name", "Cable AI Scalp"), instrument,
    ...
    if is_friday_cutoff(now_sgt, settings):
        log_event("FRIDAY_CUTOFF", f"[{instrument.replace('_', '/')}] Friday cutoff.", run_id=run_id)
        send_once_per_state(
            alert, ops, "ops_state",
        ...
        if _last_sl_at:
            ...
            if _last_sl_dt and (now_sgt - _last_sl_dt).total_seconds() < _sl_gap_min * 60:
                ...
                send_once_per_state(
                    ...
                    f"⏳ [{instrument.replace(chr(95),chr(47))}] SL cooldown — {_rem} more min.",
                    instrument)
                update_runtime_state(
    ...
    if cooldown_until:
        ...
        send_once_per_state(
            ...
            f"🧊 [{instrument.replace(chr(95),chr(47))}] Cooldown — {rem} more min.", instrument)
        update_runtime_state(last_cycle_finished=now_sgt.strftime("%Y-%m-%d %H:%M:%S"),
    ...
    if open_count >= max_concurrent:
        send_once_per_state(
            ...
            f"⏸️ [{instrument.replace('_', '/')}] Max concurrent trades ({open_count}/{max_concurrent}).",
            instrument)
        update_runtime_state(last_cycle_finished=now_sgt.strftime("%Y-%m-%d %H:%M:%S"),
...
def _signal_phase(db, run_id, settings, alert, trader, history,
    ...
    if news_penalty:
        ...
        send_once_per_state(
            ...
            msg_news_penalty(
                ...
                max_score=compute_max_score(settings),
            ), instrument)
    ...
    # Tokyo fresh-cross override (INERT at baseline): only fires when Tokyo threshold > 4.
    # At baseline all sessions = 4, so this never triggers. Retained for the case where a
    # session threshold is later raised above 4, allowing fresh crosses a lower bar.
    ...
    def _send_signal_update(decision, reason, extra_payload=None):
        payload = _signal_payload(score=score, direction=direction,
                                  ...
                                  tp_min_pct=float(settings.get("tp_min_pct", 0.35)),
                                  max_score=compute_max_score(settings),
                                  **(extra_payload or {}))
        msg = msg_signal_update(
            ...
            h4_adx=levels.get("h4_adx"),
            h4_trending=levels.get("h4_trending", True),
            max_score=compute_max_score(settings),
            **payload,
        )
    ...
    if score < _thr:
        if score >= _tg_min_score or _tg_min_score == 0:
            _send_signal_update(
                "WATCHING", f"Score {score}/{compute_max_score(settings)} below threshold ({_thr})",
    ...
    # A fresh cross where EMA9 and EMA21 are nearly identical is noise, not momentum.
    # Require minimum separation between EMAs after the cross.
    _ema_sep_min   = float(settings.get("ema_min_separation_pips", 1.5))
    _ema_pip_size  = float(levels.get("pip_size") or 0.0001)
    _ema9_val      = float(levels.get("ema{}".format(settings.get("ema_fast_period", 9))) or 0)
    _ema21_val     = float(levels.get("ema{}".format(settings.get("ema_slow_period", 21))) or 0)
    _ema_sep_pips  = abs(_ema9_val - _ema21_val) / _ema_pip_size if (_ema_pip_size and _ema9_val and _ema21_val) else 999
    if (_ema_sep_min > 0 and _ema9_val and _ema21_val
            and _ema_sep_pips < _ema_sep_min
            and "Fresh Cross" in setup):
        signal_blockers.append(
            f"EMA separation too small ({_ema_sep_pips:.1f}p < {_ema_sep_min:.1f}p min) — likely noise"
        )
        log.info("[%s] FILTER ema_separation | %.1fp < %.1fp min | score=%s/%s dir=%s setup=%s",
                 instrument, _ema_sep_pips, _ema_sep_min, score, compute_max_score(settings), direction, setup)

    # ── Stage 2: H4 ADX trend strength filter ────────────────────────────────
    # Skip entries when H4 ADX < threshold — market is ranging, not trending.
    # EMA crossover strategies only work in trending conditions.
    _adx_enabled   = bool(settings.get("h4_adx_filter_enabled", True))
    _adx_threshold = float(settings.get("h4_adx_threshold", 20.0))
    _h4_adx        = levels.get("h4_adx")
    _h4_trending   = levels.get("h4_trending", True)
    if (_adx_enabled and _h4_adx is not None and not _h4_trending):
        signal_blockers.append(
            f"H4 ADX {_h4_adx:.1f} < {_adx_threshold:.0f} — ranging market, no trend momentum"
        )
        log.info("[%s] FILTER h4_adx | %.1f < %.0f (ranging) | score=%s/%s dir=%s",
                 instrument, _h4_adx, _adx_threshold, score, compute_max_score(settings), direction)
    ...
    if (_h1_enabled and _h1_mode == "strict" and
        ...
        log.info("[%s] BLOCKED h1_strict | %s | score=%s/%s setup=%s",
                 instrument, _h1_reason, score, compute_max_score(settings), setup)
    ...
    if signal_blockers:
        log.info("[%s] BLOCKED signal_validation | %s | score=%s/%s dir=%s setup=%s",
                 instrument, " ; ".join(signal_blockers), score, compute_max_score(settings), direction, setup)
    ...
    if units <= 0:
        ...
        alert.send(msg_error(
            f"[{instrument.replace('_', '/')}] Insufficient margin",
    ...
    if units < _min_units:
        ...
        _send_signal_update(
            ...
            {"rr_ratio": rr_ratio, "tp_pct": tp_pct,
             ...
             "open_trade_ok": True, "margin_ok": False})
        alert.send(msg_error(
            f"[{instrument.replace('_', '/')}] Units too small after margin guard",
    ...
    mid, bid, ask = trader.get_price(instrument)
    if mid is None:
        alert.send(msg_error(f"[{instrument.replace('_', '/')}] Cannot fetch price", "OANDA returned None"))
        db.finish_cycle(run_id, status="FAILED",
    ...
    # change SL/TP, change size, or override the fixed calendar hard lock.
    try:
        _ai_scores = settings.get("ai_news_guard_apply_to_scores", [4, 5, 6]) or [4, 5, 6]
        _ai_scores = {int(x) for x in _ai_scores}
    except Exception:
        _ai_scores = {4, 5, 6}
    ...
    ai_guard_result = {
        ...
        "risk_level": "LOW",
        "action": "ALLOW",
        "reason": "AI News Guard disabled.",
    }
    if bool(settings.get("ai_news_guard_enabled", False)) and int(score or 0) in _ai_scores:
        _lookahead = []
        ...
        ai_payload = {
            ...
            "raw_score": raw_score,
            "direction": direction,
            "h1_filter_mode": settings.get("h1_filter_mode", "strict"),
            "h1_filter": {
                "trend": levels.get("h1_trend", "UNKNOWN"),
                "aligned": levels.get("h1_aligned", True),
            },
            "calendar_hard_lock_active": False,
            "calendar_status": news_status if isinstance(news_status, dict) else {"status": str(news_status)},
            "relevant_currencies": settings.get("news_relevant_currencies", ["GBP", "USD"]),
            "upcoming_events": _lookahead[:5],
            "setup": levels.get("setup", ""),
            "details": details,
        ...
        ai_guard_result = ai_news_guard(settings, ai_payload)
        _ai_action = str(ai_guard_result.get("action", "ALLOW")).upper()
        _ai_risk = str(ai_guard_result.get("risk_level", "LOW")).upper()
        _ai_reason = str(ai_guard_result.get("reason", ""))
        log.info("AI News Guard | risk=%s action=%s reason=%s",
                 _ai_risk, _ai_action, _ai_reason, extra={"run_id": run_id})
        db.upsert_state("last_ai_news_guard", {
            "checked_at_sgt": now_sgt.strftime("%Y-%m-%d %H:%M:%S"),
            "instrument": instrument,
            "session": session,
            "score": score,
        ...
        if _ai_action == "CAUTION":
            send_once_per_state(
                alert, ops, "ai_news_guard_state",
                f"ai_caution:{today}:{instrument}:{score}:{direction}:{_ai_reason[:40]}",
                msg_ai_guard_result(
                    action="CAUTION", risk_level=_ai_risk, reason=_ai_reason,
                    score=score, direction=direction, pair=instrument.replace("_", "/"),
                    max_score=compute_max_score(settings),
                ),
                instrument,
            )

        if _ai_action == "BLOCK" and bool(settings.get("ai_news_guard_block_action", True)):
            _reason = f"AI News Guard blocked: {_ai_risk} risk — {_ai_reason}"
            _send_signal_update(
                "BLOCKED", _reason,
                {"rr_ratio": rr_ratio, "tp_pct": tp_pct,
                 "spread_pips": spread_pips, "spread_limit": spread_limit,
            ...
            log_signal(score, direction, macro, levels, "BLOCKED_AI_NEWS_GUARD",
                       block_reason=_reason, settings=settings)
            send_once_per_state(
                alert, ops, "ai_news_guard_state",
                f"ai_block:{today}:{instrument}:{score}:{direction}:{_ai_reason[:40]}",
                msg_ai_guard_result(
                    action="BLOCK", risk_level=_ai_risk, reason=_ai_reason,
                    score=score, direction=direction, pair=instrument.replace("_", "/"),
                    max_score=compute_max_score(settings),
                ),
                instrument,
            ...
            update_runtime_state(
                last_cycle_finished=now_sgt.strftime("%Y-%m-%d %H:%M:%S"),
                status="SKIPPED_AI_NEWS_GUARD", score=score,
                direction=direction, reason=_reason,
            )
            db.finish_cycle(run_id, status="SKIPPED", summary={
                "stage": "ai_news_guard", "score": score, "direction": direction,
                "risk_level": _ai_risk, "action": _ai_action,
                "reason": _ai_reason, "instrument": instrument,
            })
            return None
