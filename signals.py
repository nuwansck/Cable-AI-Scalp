"""Signal engine for EMA crossover + ORB scalping — Cable AI Scalp v1.0
...
def _price_dp(pip_size: float) -> int:
    ...
    if pip_size <= 0.01:
        return 3   # JPY pairs (not used in Cable AI Scalp v1.0)
    return 2       # fallback
...
class SignalEngine:
    ...
    def analyze(self, instrument: str = "GBP_USD", settings: dict | None = None):
        ...
        Args:
            instrument: OANDA instrument code (GBP_USD for Cable AI Scalp v1.0)
    ...
    # -- Pip value helper (v1.0 GBP/USD only) --------------------------------
