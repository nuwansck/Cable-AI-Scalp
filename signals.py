"""Signal engine for EMA crossover + ORB scalping — Cable AI Scalp v1.2
...
Max score = sum of positive weights in settings.json["score_weights"]
(default 6: ema_fresh_cross 3 + orb_fresh 2 + cpr_bias 1).
Min threshold: signal_threshold (default 4)

All parameters — including scoring point weights — read from settings
(merged global + pair-specific).
...
# Signal scoring weights — fallback only; settings.json["score_weights"] is the
# single source of truth in production. Keep these in sync as a safety net.
...
def get_score_weights(settings: dict | None = None) -> dict:
    """Merge configured score_weights over the defaults so a partial override
    still yields a complete weight set."""
    w = dict(_DEFAULT_SCORE_WEIGHTS)
    cfg = (settings or {}).get("score_weights")
    if isinstance(cfg, dict):
        w.update({k: cfg[k] for k in w if k in cfg})
    return w


def compute_max_score(settings: dict | None = None) -> int:
    """Maximum achievable score = best EMA contribution (fresh cross, which is
    mutually exclusive with aligned) + best ORB (fresh) + CPR bias. The
    exhaustion penalty is negative and never adds to the ceiling."""
    w = get_score_weights(settings)
    return int(w["ema_fresh_cross"] + w["orb_fresh"] + w["cpr_bias"])
...
def _price_dp(pip_size: float) -> int:
    ...
    if pip_size <= 0.01:
        return 3   # JPY pairs (not used in Cable AI Scalp v1.2)
    return 2       # fallback
...
class SignalEngine:
    ...
    def analyze(self, instrument: str = "GBP_USD", settings: dict | None = None):
        """Run the Cable AI Scalp EMA + ORB (time-decayed) + CPR-bias scoring engine.

        Args:
            instrument: OANDA instrument code (GBP_USD for Cable AI Scalp v1.2)
        ...
        # -- Scoring weights (single source: settings.json["score_weights"]) --
        _w         = get_score_weights(settings)
        _max_score = compute_max_score(settings)
        ...
        _w_fresh = _w["ema_fresh_cross"]
        _w_align = _w["ema_aligned"]
        cross_tmpl = ("EMA{ef} {verb} EMA{es} | prev(" + fmt + "/" + fmt +
                      ") -> now(" + fmt + "/" + fmt + ") (+{pts})")

        if fresh_bull:
            direction = "BUY";  score += _w_fresh;  setup = "EMA Fresh Cross Up"
            reasons.append(cross_tmpl.format(
                ema_fast_prev, ema_slow_prev, ema_fast_now, ema_slow_now,
                ef=_ema_fast, es=_ema_slow, verb="fresh cross ABOVE", pts=_w_fresh,
            ))
        elif fresh_bear:
            direction = "SELL"; score += _w_fresh;  setup = "EMA Fresh Cross Down"
            reasons.append(cross_tmpl.format(
                ema_fast_prev, ema_slow_prev, ema_fast_now, ema_slow_now,
                ef=_ema_fast, es=_ema_slow, verb="fresh cross BELOW", pts=_w_fresh,
            ))
        elif bull_align:
            direction = "BUY";  score += _w_align;  setup = "EMA Trend Up"
            reasons.append(("EMA{ef}=" + fmt + " above EMA{es}=" + fmt +
                             " | aligned bull, no fresh cross (+{p})").format(
                ema_fast_now, ema_slow_now, ef=_ema_fast, es=_ema_slow, p=_w_align))
        elif bear_align:
            direction = "SELL"; score += _w_align;  setup = "EMA Trend Down"
            reasons.append(("EMA{ef}=" + fmt + " below EMA{es}=" + fmt +
                             " | aligned bear, no fresh cross (+{p})").format(
                ema_fast_now, ema_slow_now, ef=_ema_fast, es=_ema_slow, p=_w_align))
        else:
        ...
        if direction == "BUY" and current_close > pivot:
            ...
            reasons.append(("Price " + fmt + " above CPR pivot " + fmt +
                             " | bullish bias (+{p})").format(current_close, pivot, p=_w_cpr))
        elif direction == "SELL" and current_close < pivot:
            score += _w_cpr
            reasons.append(("Price " + fmt + " below CPR pivot " + fmt +
                             " | bearish bias (+{p})").format(current_close, pivot, p=_w_cpr))
        else:
        ...
        _w_exhaust = _w["exhaustion_penalty"]   # negative value
        if _exhaust_mult > 0 and atr_val and atr_val > 0 and not _orb_contributed:
            ...
            if _stretch > _exhaust_mult:
                score = max(score + _w_exhaust, 0)
                reasons.append(
                    "Exhaustion: stretch={:.2f}x ATR (>{:.1f}x) | score {:+d} -> {}/{}".format(
                        _stretch, _exhaust_mult, _w_exhaust, score, _max_score))
            else:
        ...
        _adx_enabled   = bool((settings or {}).get("h4_adx_filter_enabled", True))
        _adx_period    = int((settings or {}).get("h4_adx_period",    14))
        _adx_threshold = float((settings or {}).get("h4_adx_threshold", 20.0))
        if _adx_enabled:
            _h4 = self._get_h4_adx(instrument, _adx_period, 60, _dp)
            if _h4["h4_adx"] is not None:
                _h4["h4_trending"] = _h4["h4_adx"] >= _adx_threshold
        else:
            _h4 = {"h4_adx": None, "h4_trending": True}

        levels["h4_adx"]      = _h4["h4_adx"]
        levels["h4_trending"] = _h4["h4_trending"]
        ...
        _min_score = int((settings or {}).get("signal_threshold", MIN_TRADE_SCORE))
        if blockers:
            log.info("Signal BLOCKED | %s setup=%s dir=%s score=%s/%s blockers=%s",
                     instrument, setup, direction, score, _max_score, "; ".join(blockers))
        elif score < _min_score:
            # Sub-threshold: no trade will fire — label clearly to avoid confusion
            log.info("Signal | %s setup=%s dir=%s score=%s/%s below_threshold (would_be=$%s)",
                     instrument, setup, direction, score, _max_score, position_usd if position_usd else 0)
        else:
            log.info("Signal | %s setup=%s dir=%s score=%s/%s position=$%s",
                     instrument, setup, direction, score, _max_score, position_usd)
    ...
    # -- Pip value helper (v1.2 GBP/USD only) --------------------------------
    ...
    def _get_pip_value_usd(self, instrument: str, current_close: float,
        ...
        JPY dynamic logic removed in v2.3 — Cable AI Scalp is GBP/USD only.
        """
    ...
    def _adx(self, highs: list, lows: list, closes: list, period: int = 14) -> float:
        """Compute ADX for trend strength. Returns 0.0 on insufficient data.
        ADX > 25 = strong trend. ADX < 20 = ranging/weak market.
        """
        try:
            n = len(closes)
            if n < period + 2:
            ...
            plus_di  = [100*p/a if a else 0 for p, a in zip(plus_s,  atr_s)]
            minus_di = [100*m/a if a else 0 for m, a in zip(minus_s, atr_s)]
            dx = [abs(p-m)/(p+m)*100 if (p+m) else 0 for p, m in zip(plus_di, minus_di)]
            if len(dx) < period:
                return 0.0
    ...
    def _get_h4_adx(self, instrument: str, period: int = 14, count: int = 60, dp: int = 5) -> dict:
        """Fetch H4 candles and compute ADX for trend strength.
        ADX >= 20 = trending (allow trades). ADX < 20 = ranging (block trades).
        Fail-open: if fetch fails, returns trending=True so trades are not blocked.
        """
        try:
            closes, highs, lows = self._fetch_candles(instrument, "H4", count)
            if len(closes) < period + 2:
                return {"h4_adx": None, "h4_trending": True}
            adx_val = self._adx(highs, lows, closes, period)
            return {"h4_adx": adx_val, "h4_trending": adx_val >= 20.0}
        except Exception as exc:
            import logging as _log
            _log.getLogger(__name__).warning("H4 ADX fetch failed: %s", exc)
            return {"h4_adx": None, "h4_trending": True}
